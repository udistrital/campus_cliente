...
export class ListTablaNotasComponent implements OnInit {
  ...
  loadData(): void {
    this.idiomaService.get('tabla_notas/?limit=0').subscribe(res => {
      if (res !== null) {
  ...
  onDelete(event): void {
    ...
    .then((willDelete) => {
      ...
      if (willDelete.value) {
        this.idiomaService.delete('tabla_notas/', event.data).subscribe(res => {
          if (res !== null) {
            this.loadData();
            this.showToast('info', 'deleted', 'TablaNotas deleted');
            }
         });
      }
    });
